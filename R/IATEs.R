...
IATEs_original<- function(y_t,d_t,x_t,tau_t,x_v) {
...
#' @param d_t Vector of training treament indicators
#' @param x_t Matrix of training covariates (N x p matrix)
#' @param x_v Matrix of validation covariates (J x p, validation sample)
#' @param K Integer of the number of splits
#'
#' @return Returns a list with length 11 which contains the IATEs of the validation sample

IATEs <- function(y_t, d_t, x_t, x_v, K = 2) {
  ...
  # Estimate RF nuisance parameters
  index <- caret::createFolds(y_t, k = K)
  ...
  names(iates_list) <-  c("RF CMR","RF MOM IPW","RF MOM DR","CF","CF LC",
